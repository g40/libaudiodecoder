...
int AudioDecoderMediaFoundation::open()
{
	...
	*/
	...
	hr = MFCreateSourceReaderFromURL(m_wfilename.c_str(), NULL, &m_pReader);
	if (FAILED(hr))
	{
...
int AudioDecoderMediaFoundation::seek(int sampleIdx)
{
	if (sDebug)
	{
		std::cout << "seek() " << sampleIdx << std::endl;
	}
	PROPVARIANT prop;
	HRESULT hr(S_OK);
...
int AudioDecoderMediaFoundation::read(int frames, std::vector<float*>& buffer)
{
	...
	for (int frame = 0; frame < frames; frame++)
	{
		...
		{
			buffer[channel][frame] = *destBuffer++ / sampleMax;
		}
	}
	return frames;
}
...
{
	assert(size < sizeof(m_destBuffer));
	if (sDebug)
	{
	...
	while (!m_dead && framesNeeded > 0)
	{
		...
		if (m_seeking)
		{
			...
			if (m_nextFrame < bufferPosition)
			{
				...
				short* pBufferCurpos = destBuffer +
...
releaseSample:
	...
	if (sDebug)
	{
		std::cout << "read() " << size << " returning " << samples_read << std::endl;
	}
	// JME make this optional as we'd rather de-interleave without the addiitonal copy
	if (destination)
	{
